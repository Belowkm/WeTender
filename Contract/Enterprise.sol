...
contract Enterprise {
    ...
    function Build(uint ProjectID, uint money, string remark) public onlyOwner {
        require(
            Project(proj[ProjectID]).getStatus() == 1 ||
                Project(proj[ProjectID]).getStatus() == 2 ||
                Project(proj[ProjectID]).getStatus() == 6,
            "非法操作!"
        );
    ...
    function getTraceInfo(
        uint ProjectID
    ) public view returns (Project.TraceData[] memory _data) {
    ...
    //物料立项
    function CreateProj_etp(
        uint ProjectID,
        uint ProjectID_etp,
        string ProjectName_etp,
        uint money,
        string remark
    ) public onlyOwner {
        Build(ProjectID, money, remark);
        proj_etp[ProjectID_etp] = new Project(
            ProjectID_etp,
            ProjectName_etp,
            money
        );
